...
export class UpdateWave {
      ...
      for (let enemy of this.scene.nodes) {
        if (enemy != null && enemy.role == "enemy") {
          ...
          if (enemy.hp <= 0) {
            ...
            this.money += Math.min(enemy.maxHp / 5 * 1.5, 50);
            this.playDeath();
          ...
          let move = 0.05;
          let res = this.get_path_position(
          ...
          if (
            this.distance(enemy.translation, [this.end[0], 0, this.end[1]]) <
            0.1
          ) {
  ...
  update_turret(scene, turret, path) {
    ...
    if (turret.target) {
      if (
        this.distance(turret.target.translation, turret.translation) > RANGE ||
    ...
    let targetLocation = [
      turret.target.translation[0],
      turret.target.translation[2],
    ];
  ...
  get_path_position(distance_to_travel, path) {
    ...
    for (let it of path) {
      ...
      let segment_len = Math.max(
        Math.abs(it[0] - prevIt[0]),
        Math.abs(it[1] - prevIt[1])
      );
    ...
    if (this.scene != null) {
      for (let bullet of this.scene.nodes) {
        if (bullet != null) {
          if (
            bullet.role == "bullet" &&
            this.scene.nodes.indexOf(bullet.target) == -1
          ) {
            this.scene.removeNode(this.scene.nodes.indexOf(bullet));
          }
        }
